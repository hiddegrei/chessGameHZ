...
public class Game {
	 ...
	 * 
	 * It checks wether the active player as a active position or not. 
	 * If the active player does not have a active position it tries to set a new active position.
	 * If the active player already has a active position it fires a function to check if a move can be made.
	...
	public void clickedOnPiece(int x, int y) {
		...
		if((playerAtTurn.getActivePosition() == null) || (playerAtTurn.getActivePosition() == position)) {
			if(position.getPiece() != null && position.getPiece().getSide() != playerAtTurn.getSide()) {
				return;
			}
